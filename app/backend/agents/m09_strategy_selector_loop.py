...
def create_inputs_node(state: State):
    ...
    print("\n[PLANNER] Creating initial IT request data...")
    # Force an imperfect start for the demo
    return {
        "payload": {"employee_id": "E-102"}, # Missing department
...
def execute_node(state: State):
    ...
    try:
        ...
        return {"backend_result": b_res, "policy_result": p_res, "error": None, "attempts": it}
    
    except Exception as e:
        # Determine error origin
        error_type = "payload_problem" if "BACKEND" in str(e) else "tool_query_problem"
        print(f"[CRASH] {str(e)}")
        return {"error": {"type": error_type, "msg": str(e)}, "attempts": it}
...
def diagnose_node(state: State):
    ...
    print(f"[DIAGNOSIS] Strategy: {outcome.strategy} | Why: {outcome.justification}")
...
def main():
    ...
    builder.add_node("finish", lambda s: print("\n[END] Workflow complete.") or {})

    # Edges
    builder.add_edge(START, "create_inputs")
    builder.add_edge("create_inputs", "execute")
    
    # Loop from specialists back to execution
    ...
    builder.add_conditional_edges(
        "execute", 
        route_next_steps, 
        {"diagnose": "diagnose", "finish": "finish"}
    )
    ...
    print("\n--- Starting Mission 7 - Project 09 (Master Brain Orchestration) ---")
    app.invoke({
...
def run_agent(user_input: str):
    return app.invoke({"logs": [], "user_input": user_input, "payload": None, "policy_query": None, "attempts": 0, "error": None, "strategy": None, "backend_result": None, "policy_result": None})
